...
def get_embedding(text: str, model: str = "qwen3-embedding:0.6b") -> np.ndarray:
    try:
...
def load_existing_data() -> Tuple[List[SentenceWithSource], np.ndarray, faiss.Index, Set[str]]:
...
def save_data(sentences: List[SentenceWithSource], embeddings: np.ndarray, index: faiss.Index, processed_files: set, model: str = "qwen3-embedding:0.6b"):
